...
module String =
  ...
  let inline padl (totalWidth: int, paddingChar: char) (s: string) = if s = null then null else s.PadLeft(totalWidth, paddingChar)
  let inline padr (totalWidth: int, paddingChar: char) (s: string) = if s = null then null else s.PadRight(totalWidth, paddingChar)
