...
module String =
  ...
  let inline replace (oldValue: string, newValue: string) (s: string) = if isEmpty s then null else s.Replace(oldValue, newValue)
  let inline padLeft (totalWidth: int, paddingChar: char) (s: string) = if isEmpty s then null else s.PadLeft(totalWidth, paddingChar)
  let inline padRight (totalWidth: int, paddingChar: char) (s: string) = if isEmpty s then null else s.PadRight(totalWidth, paddingChar)
  let inline remove (startIndex: int, count: int) (s: string) = if isEmpty s then null else s.Remove(startIndex, count)
  let inline removeAfter (startIndex: int) (s: string) = if isEmpty s then null else s.Remove(startIndex)
